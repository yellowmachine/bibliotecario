...
{#if success}
	<div class="alert alert-success">
		{success}
		{#if props.mode === 'insert'}
			<a href="/book/{props.book.id}">Ver libro</a>  <!-- Esto también da error: props.book en insert NO tiene id! Ver abajo -->
			<button class="btn btn-primary" onclick={clear}>Agregar otro libro</button>  <!-- Corrige typo: primart → primary -->
		{/if}
	</div>
{/if}
